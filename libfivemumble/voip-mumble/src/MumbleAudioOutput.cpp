...
std::shared_ptr<bool> g_use3dAudio;
std::shared_ptr<bool> g_useSendingRangeOnly;
std::shared_ptr<bool> g_use2dAudio;
std::shared_ptr<bool> g_useNativeAudio;

void MumbleAudioOutput::Initialize()
{
	g_use3dAudio = std::make_shared<bool>(false);
	g_useSendingRangeOnly = std::make_shared<bool>(false);
	g_use2dAudio = std::make_shared<bool>(false);
	g_useNativeAudio = std::make_shared<bool>(false);
...
void MumbleAudioOutput::ClientAudioState::PushPosition(MumbleAudioOutput* baseIo, float position[3])
{
	...
	if (voice)
	{
		...
		if ((position[0] != 0.0f || position[1] != 0.0f || position[2] != 0.0f))
		{
			...
			if (g_use3dAudio && baseIo->m_x3daCalculate && client->overrideVolume < 0.f && distance > 0.0f)
			{
			...
			else if (g_use2dAudio && client->overrideVolume < 0.f && distance > 0.0f)
			{
